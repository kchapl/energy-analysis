...
object Reading:
  ...
  private def parseConsumption(value: String): Either[ReadingError, Double] =
    Try(value.toDouble).toEither
      .left.map(_ => InvalidConsumption(value))
      .flatMap(v =>
        if (v >= 0) Right(v)
  ...
  private def parseDateTime(field: String, value: String): Either[ReadingError, ZonedDateTime] =
    Try(ZonedDateTime.parse(value))
      .toEither
      .left.map(_ => InvalidDateTime(field, value))
  ...
  def electricityUse(readings: List[Either[ReadingError, Reading]])(start: LocalDate, end: LocalDate): Kwh =
    readings
